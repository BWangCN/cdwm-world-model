...
- **Non-degenerate tv_max: median 0.28, 46% > 0.3, 28% > 0.5.** A substantial subpopulation (elongated / asymmetric)
...
- Round objects (orange, baseball) remain metric edge-cases (sphere collapse: n_eff≈1, tv≈1 but meaningless) — refine the
  degeneracy flag beyond `n_eff>8` before quoting per-object numbers for those.
