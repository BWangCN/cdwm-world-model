...
from common.paths import GRASP_ROOT as HF
_MANI = _SHAPE = FMEAN = FSTD = None                          # lazy (see _ensure): the module imports without data present


def _ensure():                                                # load manifest + per-channel feat stats on first use
    global _MANI, _SHAPE, FMEAN, FSTD
    if _MANI is not None: return
    _MANI = json.load(open(f"{HF}/manifest.json"))
    _SHAPE = {m["object_id"]: m["shape_class"] for m in _MANI}
    _fs = np.load(f"{C.RUN}/feat_stats.npz"); FMEAN, FSTD = _fs["mean"], _fs["std"]
