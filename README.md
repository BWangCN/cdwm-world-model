...
**Start here → [`notes/00_overview.md`](notes/00_overview.md)** (shared encoder, honest-eval protocol, dataset), then:
- [`notes/01_rigid_gripper_contraction.md`](notes/01_rigid_gripper_contraction.md) — object tilt under gripper closing (DiT diffusion head).
- [`notes/02_slip_contact_dynamics_wm.md`](notes/02_slip_contact_dynamics_wm.md) — slip/drop world model (DiT rollout) + coupling validations.

## Layout
- `wm/` — shared model package (local encoder + DiT / classifier / rollout heads).
- `train_*.py` · `eval_*.py` · `compute_traj_*.py` — training, evaluation, precompute.
- `fig5_rollout.py` · `render_rollouts.py` — result figures + rollout clips.
- `notes/` — design, experiments & results, terminology, paths (theme notes `00`–`02` + dev logs).  `figures/` — figures + representative rollout clips.
