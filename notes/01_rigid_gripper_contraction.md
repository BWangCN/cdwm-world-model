...
## Rollout animation (the DiT diffusion imagining the closing tilt)
![Grasp DiT rollout — the model imagines the H=32 closing trajectory; N=8 samples cluster tight on GT](../figures/rollout_grasp.gif)

*The best grasp WM `local_geo` (DiTWM, H=32 DDPM ε-prediction + geodesic aux) rolls out the full gripper-closing trajectory for a held-out object (Perricone moisturizer, net tilt 16.7°): the **object 3DGS cloud + blue oriented box follow the ground-truth tilt**, the model's **N=8 imagined samples = orange boxes**. Because a successful grasp has **one reliable future**, the samples cluster **tight** on the GT (median single-shot sample geo **4.41°** on this clip, ≈ the 3.46° test number) — the visual counterpart of the deterministic-outcome result, and the deliberate contrast to the slip/drop rollouts ([02](02_slip_contact_dynamics_wm.md)) whose sample clouds fan out with outcome uncertainty. Rendered by `render_grasp_roll.py` (GPU sampling → `figures/rollout_grasp.{mp4,gif}`). Full-res: [rollout_grasp.mp4](../figures/rollout_grasp.mp4).*
